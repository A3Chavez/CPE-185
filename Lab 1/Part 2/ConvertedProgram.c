...
int main() {
    ...
    int DL = 20,         // Register DL ; MOV DL, ##  ; Set Water Filling Rate
        BL = mem1[0],    // Register BL ; MOV BL,[##] ; Get Current Water Level from memory
        CL = mem1[1];    // Register CL ; MOV CL,[##] ; Get Irrigation Amount from memory
